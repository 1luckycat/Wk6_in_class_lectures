...
# instantiate our Blueprint class
                                # this is keyword arg     location of html files (shop.html)
...
def shop():
    ...
    shop_stats = {
        ...
        'customers': len(allcustomers), # total number of customers
        'sales': sum([order.order_total for order in allorders])  # looping thru allorders list, order is each object and im grabbing the order_total & summing them all up
    }
    ...
    return render_template('shop.html', shop=allprods, stats=shop_stats) # looking inside site_templates folder for a file called shop.html to render
                                        # this shop is from for product in shop loop.html
...
@site.route('/shop/update/<id>', methods=['GET', 'POST'])  #<id> is a parameter to show which specific product we are updating
                                                         # <parameter> this is how we pass parameters to our routes
def update(id):
    ...
    if request.method == 'POST' and updateform.validate_on_submit():
        ...
        return redirect(f'/shop/update/{product.prod_id}')
                                            ...
                                            # left of = is html, right of = is what its referencing in function (product=product)
    return render_template('/update.html', form=updateform, product=product)


@site.route('/shop/delete/<id>')
def delete(id):
    # query the database to find that product
