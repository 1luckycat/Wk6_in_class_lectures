...
from flask_login import LoginManager, UserMixin  # LoginManager - logins in, UserMixin - class  / load user as current_user and class to help us do that
from datetime import datetime # put in timestamp when we create any new object in the database
...
class User(db.Model, UserMixin):
    ...
    last_name = db.Column(db.String(30))  # dont need nullable - dont need first and last name to sign up
    username = db.Column(db.String(30), nullable=False, unique=True)  # nullable - username must be unique and used
    email = db.Column(db.String(150), nullable=False, unique=True) # cant sign up with the same email
    password = db.Column(db.String, nullable=False)
    date_added = db.Column(db.DateTime, default=datetime.utcnow()) # very similar to default CURRENT_DATE in postgres
    ...
    def set_id(self):
        return str(uuid.uuid4())  # uuid4 makes it a certain way for our primary key / creating random string for our primary key
    ...
    def __repr__(self):
        return f"<User: {self.username}>"  # just keeping track that username was added
...
class Product(db.Model):
    ...
    # establishing relationship between ProdOrder and Product table
    prodord = db.relationship('ProdOrder', backref = 'product', lazy=True) # lazy=True is you dont need to HAVE a relationship between them
    ...
    # when a customer buys a product we need to cdecrement the total quantity available
...
# only need this much for purpose of tracking what customers are tied to which orders
class Customer(db.Model):
    ...
    def __init__(self, cust_id):
        self.cust_id = cust_id  # this is coming from the frontend so should be the same when they create it
...
class ProdOrder(db.Model):
    ...
    # first instance of using a foreing key that is a primary key on another table
    ...
    def __init__(self, prod_id, quantity, price, order_id, cust_id):
        ...
        self.quantity = quantity  # how much quanitity of that product the customer is buying
...
# create our Schema class (aka what are data will look like when we pass it to the frontend)
# data also cannot be an object but rather a dictionary (json)
